...
pub fn sys_exec(path: *const u8) -> isize {
    ...
    // Load application from file system
    if let Some(inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        let data = inode.read_all();
        let data_leaked = Box::leak(data.into_boxed_slice());
        let task = current_task().unwrap();
        task.exec(data_leaked);
        0
    } else {
        -1
    }
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        ...
        // 直接创建新任务
        let new_task = Arc::new(TaskControlBlock::new(data_leaked));
        let new_pid = new_task.pid.0;
